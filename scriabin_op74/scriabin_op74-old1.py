...
# To do: set up to play all five tracks one after the other
#  - parse all 5 MIDI files
#  - in the draw method, choose the right files, colours and time point
#  - fade in and out at the start of each file
#  - add offset at start to make room for credits
#  - check that we haven't broken the ability to make the video!
#
# Need to fix:
...
MIDI_FILES = [
 ...
 '/home/alex/midi/python_animations/midi/scriabin_op74/2016-12-Scriabin_op74_no5_v04.mid'
]
WAV_FILE_ORIGINAL = '/audio/2013-nov-concert/scriabin_all.wav'
WAV_FILE_TEMP = '/tmp/animation_audio.wav' # temporary store for padded version of audio
LENGTH = 345 # length of the video file to be generated, in seconds
START_TIMES = [0, 54.5, 136, 169, 287]
AUDIO_OFFSET = 0 # number of seconds late to start the audio
MIDI_OFFSETS = [1, -0.5, -1.2, -1.3, -3] # number of seconds to shift midi events by
TRACK_ORDERS = [
  [2, 3, 4],
  [2, 3, 4, 5, 6],
  [2, 3, 4, 5, 6, 7],
...
OUTPUT_FILE = '/home/alex/midi/python_animations/scriabin_preludes_op74.mp4'

TITLE_TEXT = "Scriabin: Five preludes, opus 74"
...
FADE_OUT_TIME = 4 # number of seconds to fade out/in at the end/beginning of each piece
...
def draw_note(n):
  ...
  pygame.draw.rect(screen, dimmer(n.col, FADE_OUT), rescale(noterect, n.z))
...
def make_frame(t):
    ...
    if k<len(START_TIMES)-1:
      ITEM_LENGTH = START_TIMES[k+1] - START_TIMES[k]
      boundary_time = min(START_TIMES[k+1]-t, t-START_TIMES[k])
    else:
      ITEM_LENGTH = LENGTH - START_TIMES[k]
      boundary_time = max(min(LENGTH-t, t-START_TIMES[k]), 0)
    if boundary_time < FADE_OUT_TIME:
      FADE_OUT = boundary_time / FADE_OUT_TIME
    else:
      FADE_OUT = 1
