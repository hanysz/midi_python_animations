...
LENGTH = 200 # length of the video file to be generated, in seconds
AUDIO_OFFSET = 7 # number of seconds late to start the audio
MIDI_OFFSET = -0.256 # number of seconds to shift midi events by, for video
MIDI_OFFSET = -0.4 # number of seconds to shift midi events by, for video
# adjust because it looks better if the MIDI leads the audio by a tiny amount!
#MIDI_OFFSET = -7 # number of seconds to shift midi events by for live playback
TRACK_ORDER = [2, 3]
OUTPUT_FILE = '/home/alex/midi/python_animations/scarlatti_K440.mp4'

# Adjust offsets to synchronise with saved MIDI file
#AUDIO_OFFSET = 7.256 # number of seconds late to start the audio
#MIDI_OFFSET = 0 # number of seconds to shift midi events by

FPS = 25 # frames per second for saved video output
#FPS = 8 # for quick testing
...
XDRIFT_SPEED = -30 # how fast the current carries the ripples to the right
YDRIFT_SPEED = 11
FADEOUT_TIME = 12 # number of seconds for a note to disappear after it's stopped sounding
FADE_EXPONENT = 4 # 0 = no fade, 1 = linear fade, 2 = quadratic fade, etc
...
HEIGHT = 720
#HEIGHT = 396 # for quick testing
#HEIGHT = 198 # for quick testing
...
def draw_bubble(n, t):
  ...
  if t<appear_time or t>vanish_time:
    ...
    outline_fade = (1 - (t-n.t1) / (vanish_time - n.t1))**FADE_EXPONENT
  else:
    outline_fade = 1
  #addcol = NOTE_COLOURS[n.note % 12]
  #addcol = dimmer(addcol, outline_fade)
  addcol = ADD_COLOURS[n.note % 12]
  addcol = dimmer(addcol, outline_fade)
  subcol = SUB_COLOURS[n.note % 12]
...
if MODE == 'play':
  ...
  animation_clip = VideoClip(lambda t: make_frame(t, draw_bubble),
                             duration=LENGTH)
  titles = TextClip(
    "Sonata in B flat, K440 by Domenico Scarlatti\n\n" +
    "Performed by Alexander Hanysz\nLive recording, Pilgrim Church, Adelaide\n" +
    "10th November 2013",
    font='Segoe-Script-Regular', fontsize = 30, color = 'black'
    #font='Segoe-Script-Regular', fontsize = 20, color = 'black'
  )
  titles = titles.set_pos('center').set_duration(7.5).fadein(3, BACKGROUND
                        ).fadeout(2.5, BACKGROUND)
